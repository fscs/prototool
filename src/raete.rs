...
pub fn fetch_abmeldungen(api_url: &Url, client: &Client) -> Result<Vec<Abmeldung>> {
   let endpoint = api_url.join("api/abmeldungen/next_sitzung/")?; 
    ...
    let response = client.get(endpoint).send().context("unable to fetch abmeldungen")?;

    let abmeldungen = response.json().context("unable to deserialize abmeldungen")?;
...
mod tests {
    ...
    use uuid::Uuid;
    use super::{Rat, Person, Abmeldung};
